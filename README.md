...
When the book-specific recipe is done, we can create a spec for it.

1) Create an entry on the `spec/main_spec.rb` file like:
  ```ruby
...
2) Create a folder under `spec/books/{book-name}` with a file called `input.xhtml` and another file called `expected_output.xhtml` inside it, the `input` file contain the content inside the `assembled.xhtml` file and the `expected_output` file should contain the content inside the `normalized` version of the kitchen-baked file, so `kitchen-baked.normalized.xhtml`, both of these files are generated by the shorten script.
