...
RSpec.describe Kitchen::Directions::MoveExercisesToEOC::V3 do
  ...
  context 'when exercises are in module with titles chilren' do
    it 'keeps the modules titles children while creating a section title' do
      described_class.new.bake(chapter: book_with_review_exercises_in_module_with_title_children.chapters.first, metadata_source: metadata_element, append_to: append_to, klass: 'CLASSNAME')
      expect(append_to).to match_snapshot_auto
    end
  end
end
