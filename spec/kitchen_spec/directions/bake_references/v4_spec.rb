...
RSpec.describe Kitchen::Directions::BakeReferences do
  ...
  it 'v4 works with units with unit numbering disabled' do
    described_class.v4(
      book: book_with_units,
      metadata_source: metadata_element,
      chapters: book_with_units.units.chapters)
    expect(book_with_units.first('.os-references-container').to_s).to match_snapshot_auto
  end

  it 'v4 works with units with unit numbering enabled' do
    described_class.v4(
      book: book_with_units,
      metadata_source: metadata_element,
      chapters: book_with_units.units.chapters,
